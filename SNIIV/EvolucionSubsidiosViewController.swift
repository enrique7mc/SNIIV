...
class EvolucionSubsidiosViewController: BaseUIViewController, UIPopoverPresentationControllerDelegate  {
    
    
    
    var datos: DatosEvolucionSubsidios?
    var intEstado:Int = 0
    ...
    override func viewDidLoad() {
        ...
        if !isDataLoaded() && Reachability.isConnectedToNetwork(){
            print("DATA IS LOADED", terminator: "")
            let parseEvolucionSubsidios = ParseEvolucionSubsidios<Evolucion>()
            parseEvolucionSubsidios.getDatos(handler)
            return
        }
        else{
            print("no DATA IS LOADED", terminator: "")
    ...
    @IBAction func changeOption(sender: AnyObject) {
        ...
        switch btnChange.selectedSegmentIndex{
        case 0:
            opt = 0
        case 1:
            opt = 1
    ...
    func handler (responseObject: Evolucion, error: NSError?) -> Void {
        
        print("Handleer", terminator: "")
        if error != nil {
            print("EVOLUCION SUBSIDIOS error obteniendo datos", terminator: "")
            return
        }
    ...
    func loadFromStorage(estado: Int, option: Int) {
        print("Evolución SUBSIDIOS loadFromStorage", terminator: "")
        datos = DatosEvolucionSubsidios()
    ...
    override func getKey() -> String {
        return EvolucionSubsidiosRepository.TABLA
    }
