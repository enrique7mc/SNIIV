...
class EvolucionFinanciamientosViewController: BaseUIViewController, UIPopoverPresentationControllerDelegate {
    
    
    var datos: DatosEvolucionFinanciamientos?
    var intEstado:Int = 0
    ...
    override func viewDidLoad() {
        ...
        if !isDataLoaded() && Reachability.isConnectedToNetwork(){
            let parseEvolucionFinanciamientos = ParseEvolucionFinanciamiento<Evolucion>()
            parseEvolucionFinanciamientos.getDatos(handler)
            return
        }
        else{
    ...
    func handler (responseObject: Evolucion, error: NSError?) -> Void {
        if error != nil {
            print("EVOLUCION FINANCIAMIENTOS error obteniendo datos", terminator: "")
            return
        }
    ...
    func loadFromStorage(estado: Int, option: Int) {
        print("Evolución Financiamientoss loadFromStorage", terminator: "")
        datos = DatosEvolucionFinanciamientos()
    ...
    override func getKey() -> String {
        return EvolucionFinanciamientoRepository.TABLA
    }
